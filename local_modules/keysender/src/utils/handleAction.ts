...
function handleAction<S, R>(this: _Hotkey<S, R>, options: HotkeyOptions<_Hotkey<S, R>, S, R>) {
  ...
  if (mode === 'once') {
    const action: OmitThisParameter<typeof options.action> = options.action.bind(this)
  ...
  if (options.after) {
    const fn: OmitThisParameter<NonNullable<typeof options.after>> = options.after.bind(this)

    after = () =>
      fn(outerToggler ? (innerToggler ? Reason.BY_ACTION : Reason.BY_KEYBOARD) : reason!)
  } else {
    after = noop
  }

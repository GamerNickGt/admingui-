...
const FillChartData = (data: PlayerDetails, setChartData: (data: ChartData[]) => void) => {
  setChartData(
    ['experienceFootman', 'experienceArcher', 'experienceVanguard', 'experienceKnight'].map(
      (key) => {
      ...
      api.fetchPlayerData(player.playfabId).then((res) => {
        if (res) {
          if (res.status === 200) {
            ...
            const weapon_stats = Object.entries(stats)
              .filter(([key, _]) => key.startsWith('experienceWeapon') && !key.endsWith('Position'))
              .sort((a, b) => b[1] - a[1])
            ...
            <div className="flex-1 pb-0">
              <div className="p-2 mt-2 bg-chart-1/10 border-2 border-dashed rounded-sm border-chart-1/50">
                <div className="flex flex-row gap-2 items-center">
                  ...
                  <Badge className="text-xs bg-blue-500 hover:bg-blue-400 text-foreground font-normal rounded-md">
                    {level}
                  </Badge>
                </div>
                <div className="flex flex-row gap-2 items-center">

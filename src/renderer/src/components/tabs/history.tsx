...
import { commandHistory, hasHistoryInitialized } from '@/main'
...
function CommandCard({ savedCommand }: { savedCommand: SavedCommand }) {
    ...
    <>
      ...
      {'player' in command &&
        renderData(
          ...
          `${command.player.displayName} (${command.player.playfabId})`
        )}
      ...
      {'duration' in command &&
        ...
        renderData(
          ...
          `${command.duration} hour${command.duration > 1 ? 's' : ''}`
        )}
  ...
  return (
    <div className="bg-background rounded-lg border p-4 my-2">
      <Badge className="select-all">
        {CommandTypeMap[command.type]} @ {date}
      </Badge>

    ...
    <div className="mx-10">
      <div className="flex mb-2">
        <div className="w-full">
          <PresetSelector
            onChange={(selected) => setPresets(selected as Announcement[])}
            presetKey="announcements"
        ...
        <Button
          variant="outline"
          className="rounded-l-none group px-16"
          onClick={() => sendPresets()}
        >
          Send
      ...
      <div className="my-2">
        <Textarea
          ...
          className="max-h-96 rounded-b-none border-b-0"
          value={message}
          onChange={(e) => setMessage(e.target.value)}
        />
        <div className="flex">
          <ComboBox
            className="w-full rounded-t-none rounded-r-none"
            options={['admin', 'server']}
            onChange={(value) => setType(value as 'admin' | 'server')}
          />
          <Button
            variant="outline"
            className="w-full group rounded-l-none rounded-t-none"
            onClick={() => sendMessage(type, message)}
          >
            Send
            <SendHorizonal className="ml-1 animate-ease-in-out animate-infinite animate-duration-1000 group-hover:animate-pulse" />
          </Button>

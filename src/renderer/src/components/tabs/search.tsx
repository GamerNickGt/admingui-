...
function Search() {
  ...
  function ProcessTableData(data: PlayFabDetails, search_term?: string): TableColumns {
    ...
    const truncated = truncateAliasHistory(
      player.aliasHistory,
      search_term ? search_term : player.current_name
    )
    player.truncated_aliasHistory =
      typeof truncated === 'string' ? truncated : truncated.truncatedAliasHistory

...
import { Check, ChevronsUpDown, Edit2, Lock } from 'lucide-react'
...
export function PresetSelector<T>({
  ...
  const toggleValue = (value: Preset<T>) => {
    setSelected((curr) =>
      curr.includes(value) ? curr.filter((l) => l.label !== value.label) : [...curr, value]
    )
  ...
  const listValues = defaultValues?.concat(values)
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              ...
              {selected.length === 2 && selected.map(({ label }) => label).join(', ')}
              {selected.length > 2 && `${selected.length} Presets selected`}
            </span>
            ...
            >
              <CommandGroup className="overflow-auto">
                {listValues?.map((value) => {
                  ...
                  const isDefault = defaultValues.includes(value)
                  return (
                    <CommandItem
                    ...
                    >
                      ...
                      />
                      <div className={cn('', isDefault ? 'flex gap-2' : 'flex-1')}>
                        {value.label}
                        {isDefault && <Lock />}
                      </div>
                    </CommandItem>
...
function CommandItemCreate<T>({ inputValue, values, onSelect }: CommandItemCreateProps<Preset<T>>) {
  const hasNoValue = !values
    .map((value) => {
      return value?.label ? value.label.toLowerCase() : ''
    })
        ...
        <AccordionContent className="p-2">
          <Form {...form}>
            ...
            >
              ...
              <Button type="submit" variant="outline" size="xs" className="mt-2">
                Save
              </Button>
            </form>
          </Form>
